...
const Services = () => {
    ...
    useEffect(() => {
        ...
        const priorityServices = services.filter(service => priorityTitles.includes(service.title));
        const otherServices = services.filter(service => !priorityTitles.includes(service.title));
        const randomOthers = otherServices.sort(() => 0.5 - Math.random()).slice(0, 4 - priorityServices.length);
    ...
    return (
        <section
            id="services"
            className="container bg-[#f9f9f9] dark:bg-color1 text-color2 dark:text-color5 py-12"
        >
