                ...
                <div>
                    <div data-aos="zoom-in" data-aos-once="true" className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {products.map((product) => (
                            ...
                            >
                                <div className=" aspect-w-16 aspect-h-9 flex items-center justify-center rounded-tl-3xl rounded-br-3xl overflow-hidden">
                                    <img src={product.imageUrl} alt={product.title} className="w-40 h-40 object-contain" />
                                </div>
                                <div className="mt-4 flex-1 flex flex-col justify-between">
                                    <h2 className="text-2xl font-semibold text-[#E76F51] dark:text-[#E9C46A]">{product.title}</h2>
                                    <p className="text-lg text-[#1E293B] dark:text-white mt-2 font-poppins line-clamp-4 leading-relaxed">
                                        {product.description}
                                    </p>
